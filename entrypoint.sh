...
# then exec moq-relay with --cluster-root derived from LEADER_URL so
...
# moq-relay's --cluster-root wants a host:port pair, so default to :4443
# (the QUIC listener port the leader's relay uses) if the URL didn't
# carry one.
